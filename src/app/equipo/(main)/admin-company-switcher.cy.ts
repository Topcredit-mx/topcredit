...
describe('Admin company switcher', () => {
	...
	it('admin sees Vista general option and all active companies in switcher', () => {
		...
		cy.get('[role="menu"]').within(() => {
			cy.contains('[role="menuitem"]', 'Vista general').should('exist')
			cy.contains('[role="menuitem"]', companyAName).should('exist')
			cy.contains('[role="menuitem"]', companyBName).should('exist')
		})
	})

	it('admin can select a company and sees company name in trigger', () => {
		openCompanySwitcher()
		cy.contains('[role="menuitem"]', companyAName).should('be.visible').click()

		cy.get('#company-switcher-trigger').should('contain', companyAName)
	})

	it('selected company persists after page reload', () => {
		openCompanySwitcher()
		cy.contains('[role="menuitem"]', companyBName).should('be.visible').click()

		cy.get('#company-switcher-trigger').should('contain', companyBName)
	...
	it('admin can select Vista general to return to equipo overview', () => {
		...
		openCompanySwitcher()
		cy.contains('[role="menuitem"]', 'Vista general')
			.should('be.visible')
			.click()

		cy.get('#company-switcher-trigger').should('contain', 'Vista general')
