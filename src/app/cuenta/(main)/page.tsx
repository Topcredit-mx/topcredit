...
export default async function CuentaHomePage() {
	...
	if (applicationsList.length === 0) {
		redirect('/cuenta/applications/new')
	}
	...
	return (
		<div
			className={cn(
				shell.applicantMainMax,
				'space-y-12 pt-4 pb-12 sm:space-y-16 sm:pt-8 sm:pb-16',
			)}
		>
			<div className="flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
				<div className="min-w-0 space-y-2">
					<h1 className="font-semibold text-3xl text-brand tracking-tight sm:text-4xl">
						{tCuenta('executive-overview')}
					</h1>
					<p className="max-w-2xl text-base/7 text-slate-600">
						{tCuenta('overview-health-greeting', { name: displayName })}
					</p>
				</div>
				<div className="flex shrink-0 items-center self-end sm:self-start">
					<Button
						...
						aria-label={tCuenta('notifications-aria')}
					>
			...
			{!emailVerified && (
				...
				>
					...
					<span>
						{tCuenta('verify-email')}{' '}
						<Link
							href="/cuenta/settings/security"
							className="font-medium underline underline-offset-2"
						>
							{tCommon('settings')}
						</Link>{' '}
						{tCuenta('verify-email-suffix')}
					</span>
				</div>
			)}
				...
				>
					...
					>
						<Link href="/cuenta/applications/new">
							{tCuenta('request-now')} <ArrowUpRight className="ml-2 size-4" />
						</Link>
					</Button>
				</Card>

				<Card
					aria-labelledby="cuenta-applications-in-progress-heading"
					className={cn(
						shell.cuentaHeroCard,
												...
												<div className="flex min-w-0 flex-1 items-center gap-4">
													...
													<div className="min-w-0">
														<p className="font-semibold text-slate-900">
															{tCuenta('applications-in-progress-row-title', {
																id: app.id,
															})}
														</p>
														<p className="mt-0.5 text-slate-500 text-sm leading-snug">
															{tCuenta('applications-in-progress-meta', {
																when,
															})}
														</p>
													</div>
			...
			<div className="grid gap-8 lg:grid-cols-[1.65fr_1fr] lg:gap-10">
				<section
					...
					aria-labelledby="cuenta-loan-portfolio-heading"
				>
					<div className="flex flex-wrap items-end justify-between gap-3">
						<h2
							id="cuenta-loan-portfolio-heading"
							className={shell.cuentaSectionTitle}
						>
							{tCuenta('loan-portfolio-title')}
						</h2>
						<Link
							href="/cuenta/applications"
							className={cn(
								'inline-flex items-center gap-1.5 text-sm',
								shell.textLinkStrong,
							)}
						>
							{tCuenta('view-detailed-ledger')}
				...
				<section
					...
					aria-labelledby="cuenta-upcoming-activity-heading"
				>
					<h2
						id="cuenta-upcoming-activity-heading"
						className={shell.cuentaSectionTitle}
					>
						{tCuenta('upcoming-activity-title')}
					</h2>
							...
							<li key={row.day} className="flex gap-4">
								...
								>
									<span className="font-semibold text-[10px] text-brand uppercase tracking-[0.14em]">
										{tCuenta('activity-demo-month-oct')}
									</span>
					...
					<div className="mt-1 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-elevated">
						<p className="font-semibold text-brand">
							{tCuenta('pro-tip-title')}
						</p>
						<p className="mt-2 text-slate-600 text-sm leading-relaxed">
							{tCuenta('pro-tip-body')}
						</p>
					</div>
				</section>
			</div>
