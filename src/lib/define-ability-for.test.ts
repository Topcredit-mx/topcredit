...
test('hr+agent can confirmHrDeduction for assigned company, cannot confirmCreditPaymentInstallment', () => {
	const ability = defineAbilityFor({
		roles: ['agent', 'hr'],
		assignedCompanyIds: [10],
		userId: 99,
	...
	assert.equal(ability.can('confirmCreditPaymentInstallment', paymentCompany10), false)
})

test('installments+agent can confirmCreditPaymentInstallment for assigned company, cannot confirmHrDeduction', () => {
	const ability = defineAbilityFor({
		roles: ['agent', 'installments'],
		assignedCompanyIds: [10],
		userId: 99,
	})
	assert.equal(ability.can('confirmCreditPaymentInstallment', paymentCompany10), true)
	assert.equal(ability.can('confirmCreditPaymentInstallment', paymentCompany99), false)
	assert.equal(ability.can('confirmHrDeduction', paymentCompany10), false)
})

test('admin can both confirmHrDeduction and confirmCreditPaymentInstallment on any company', () => {
	const ability = defineAbilityFor({
		roles: ['admin'],
		assignedCompanyIds: [],
		userId: 1,
	...
	assert.equal(ability.can('confirmCreditPaymentInstallment', paymentCompany10), true)
	assert.equal(ability.can('confirmCreditPaymentInstallment', paymentCompany99), true)
})
