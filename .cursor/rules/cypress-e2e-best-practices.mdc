---
description: Cypress E2E standards — selectors, assertions, state, auth, Next.js streaming, test titles. Apply when writing or editing .cy.ts specs.
globs: **/*.cy.ts, **/cypress/**/*
alwaysApply: false
---
...
## Selectors — no data-testid

Do **not** use `data-testid` or `get('[data-testid="..."]')`. Use what the user sees or the accessibility tree: **visible text** (`cy.contains(...)`), **roles** (`role="..."`, `aria-label`), **semantic elements** (`main`, `nav`, `button`, `h1`).
...
- **Prefer role + name over attributes.** Follow Kent C. Dodds–style queries: e.g. `cy.contains('h2', /solicitante/i).closest('[data-slot="card-header"]').within(() => cy.contains('button', /acciones/i).click())` instead of matching on `aria-*` or `data-*` directly.
- **Use `asChild` for semantic headings.** When a UI primitive like `CardTitle` supports `asChild`, render real headings (`<h2>Solicitante</h2>`) and then query by that heading level in tests.
...
- After **navigate** (visit or click to new page): assert on something that exists **only on the destination** (e.g. `cy.contains('h1', /detalle de solicitud/i).should('be.visible')`), then wait for the element you’ll interact with.
...
// ✅ Inside row .within(): use cy.root() for the row, not cy.get('tr') (tr has no descendant tr)
...
- **Selectors**: `data-testid`, fragile class/id/tag selectors.
