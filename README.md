...
| `pnpm dev:test` | Start dev with `NODE_ENV=test` (requires `E2E_OTP_CODE`; no emails; for full login E2E) |
...
**E2E (local):** For full login flow tests (login → verify OTP in the UI), start the app with `E2E_OTP_CODE=<6 digits> pnpm dev:test` and run Cypress with the same `E2E_OTP_CODE` (e.g. `E2E_OTP_CODE=123456 pnpm cy:run`). In CI, a random 6-digit code is generated per run and passed to both the app and Cypress.
