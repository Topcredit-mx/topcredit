...
|------|--------|--------|-----------------|
| 1.1 | **Dashboard** (applicant) | If the applicant has no applications, they are redirected to **New application** (`/dashboard/applications/new`). | — |
| 1.2 | **New application** (applicant) | Applicant submits **general details** (e.g. profile, salary, bank) and uploads initial docs (e.g. ID, bank statement, payroll, proof of address) in **one submit**. Does **not** choose term nor credit amount. | Application created with status `pending`. |
