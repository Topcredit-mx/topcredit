...
|----------|--------|----------------|
| `requests` | `/equipo` (company selected) | Review `pending` applications, approve/reject docs. Set status to **approved** (→ next queue), **invalid-documentation**, or **denied**. Only pre-authorizations role can set pre-authorized. |
...
| `authorizations` | `/equipo` | Review `pre-authorized` applications (contract + docs uploaded by applicant). Authorize or deny with reason. |
