...
1. **Lock the applicant document intake flow**
   - Move the initial application submit to **general data + required document uploads** in one step.
   - Align the create-application UX, validations, and server actions around that flow.
   - Define and enforce the required initial document set and any freshness rules.
   - Update applicant copy and product docs so the initial submission explicitly includes documents.

2. **Implement the authorization stage explicitly**
...
3. **Design the post-authorization operational flow**
...
5. **Keep docs and tests aligned with the chosen workflow**
